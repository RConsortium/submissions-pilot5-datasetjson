...
oid_cols <- adae_spec$ds_vars %>%
  select(dataset, variable, key_seq) %>%
  left_join(adae_spec$var_spec, by = c("variable")) %>%
...
dataset_json(adae,
  ...
  item_oid = paste0("IG.ADAE"),
  name = "ADAE",
  dataset_label = adae_spec$ds_spec[["label"]],
  file_oid = file.path(path$adam, "adae"),
  columns = oid_cols
) %>%
  write_dataset_json(file = file.path(path$adam_json, "adae.json"), float_as_decimals = TRUE)
