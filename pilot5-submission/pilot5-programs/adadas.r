...
oid_cols <- adadas_spec$ds_vars %>%
  select(dataset, variable, key_seq) %>%
  left_join(adadas_spec$var_spec, by = c("variable")) %>%
...
dataset_json(adas,
  ...
  item_oid = paste0("IG.ADADAS"),
  name = "ADADAS",
  dataset_label = adadas_spec$ds_spec[["label"]],
  file_oid = file.path(path$adam, "adadas"),
  columns = oid_cols
) %>%
  write_dataset_json(file = file.path(path$adam_json, "adadas.json"), float_as_decimals = TRUE)
