...
oid_cols <- adsl_spec$ds_vars %>%
  select(dataset, variable, key_seq) %>%
  left_join(adsl_spec$var_spec, by = c("variable")) %>%
...
dataset_json(adsl,
  ...
  item_oid = paste0("IG.ADSL"),
  name = "ADSL",
  dataset_label = adsl_spec$ds_spec[["label"]],
  file_oid = file.path(path$adam, "adsl"),
  columns = oid_cols
) %>%
  write_dataset_json(file = file.path(path$adam_json, "adsl.json"), float_as_decimals = TRUE)
