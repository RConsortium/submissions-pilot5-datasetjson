...
sdtm_datasetjson_files <- list.files("original-sdtmdata/",
                        pattern = "\\.xpt$",
                        full.names = TRUE) |>
  # purrr::pluck(1) |>
  purrr::walk(
...
adam_datasetjson_files <- list.files("original-adamdata/",
                        pattern = "\\.xpt$",
                        full.names = TRUE) |>
  # purrr::pluck(1) |>
  purrr::walk(
    process_xpt_to_json,
    output_dir = file.path("pilot5-submission", "pilot5-input", "adamdata")
  )
