...
**3. A quick preview of the datasetjson data**

If you would like to inspect some of the available datasetjson files you can use the
below code to read it into your Global Environment

```r
library(datasetjson)
dm <- read_dataset_json(file.path(path$sdtm, "dm.json"))
dplyr::glimpse(dm)
```
...
Rows: 306
Columns: 25
$ STUDYID  <chr> "CDISCPILOT01", "CDISCPILOT01", "CDISCPILOT01", "CDISCPILOT01", "CDISCPILOT01", "CDI…
$ DOMAIN   <chr> "DM", "DM", "DM", "DM", "DM", "DM", "DM", "DM", "DM", "DM", "DM", "DM", "DM", "DM", …
$ USUBJID  <chr> "01-701-1015", "01-701-1023", "01-701-1028", "01-701-1033", "01-701-1034", "01-701-1…
$ SUBJID   <chr> "1015", "1023", "1028", "1033", "1034", "1047", "1057", "1097", "1111", "1115", "111…
$ RFSTDTC  <chr> "2014-01-02", "2012-08-05", "2013-07-19", "2014-03-18", "2014-07-01", "2013-02-12", …
...
```

**4. Using the source function, run the `pilot5-helper-fcns.r` program, 
...
**7. Clean Global Environment**

Successfully running all the ADaM and TLF programs can create a lot of objects in
the Global Environment. Below is a code snippet that you can run to clean up the
environment for easier viewing of the critical objects. The code emits a warning 
if any objects are missing from the `keep` list.

```r
# Keep only these objects in the Global Environment
keep <- c("path", "adsl", "adae", "adadas", "adlbc", "adtte", "tlf_demographic",
          "tlf_efficacy", "tlf_kmplot", "tlf_primary")

# Warn if anything in `keep` is missing from the Global Env
missing <- setdiff(keep, ls(envir = .GlobalEnv))
if (length(missing) > 0) {
  warning("Missing objects in .GlobalEnv: ", paste(missing, collapse = ", "), call. = FALSE)
}
...
These are the critical objects that are used to write out the datasetjson, tables
and figures:

* `path` - A list that sets up directories
* `adsl`, `adae`, `adadas`, `adlbc`, `adtte` - Final ADaM datasets pre-datasetjson conversion
* `tlf_demographic`, `tlf_efficacy`, `tlf_kmplot`, `tlf_primary` - Table and Plot objects pre-rendering
...
out data use Type from spec for integers and writes out the data as integer.  The issue was discussed with `datasetjson` R package team and is paraphrased below:

> Using AGE as an example, the column metadata for AGE was set to 
> integer, so datasetjson correctly wrote it out and read it back as an 
> integer in Pilot 5. The Dataset-JSON v1.1 specification allows for 
> integer, decimal, and float types, not a generic numeric type. Pilot
> used XPT format, which stores all numeric values as 8-byte floating 
> point, so there is no distinction between integer and numeric in XPT. 
> This difference is simply a result of the format change and is not a 
> meaningful issue. 

The discussion is tracked in this GitHub Issue: https://github.com/RConsortium/submissions-pilot5-datasetjson/issues/143
...
the datasetjson R package does not associate the format with the variable. The issue was discussed with `datasetjson` R package team and is paraphrased below:

> The Dataset-JSON column metadata does include DATE9. in the 
> displayFormat attribute for these date variables. However, when the 
> datasetjson R package reads the data back into R, it does not assign 
> this value to a format.sas attribute on the column. In contrast, 
> haven::read_xpt() preserves the SAS format metadata from the XPT file.

A proposed update to the datasetjson R package has been made. The discussion is tracked in this GitHub Issue: https://github.com/RConsortium/submissions-pilot5-datasetjson/issues/143
...
<<<<<<< HEAD
are read in.  We do not think this is an issue. However, we have discussed with dataset R package team and fix is being discussed. See GitHub Issue: https://github.com/atorus-research/datasetjson/issues/76
=======
are read in. The issue was discussed with `datasetjson` R package team and is paraphrased below:

> The warning is produced when `datasetjson::read_dataset_json()` applies
> `as.double()` to numeric columns that contain missing values recorded as
> character strings such as `"NA"`. R correctly converts those values to
> missing (`NA`) but also emits the standard "NAs introduced by coercion"
> warning during the conversion.

The discussion is tracked in this GitHub Issue: https://github.com/atorus-research/datasetjson/issues/76
>>>>>>> 6f8d656f2c9eab08337bac3308b540ee8e7fcd15
...
are read in.  We do not think this is an issue. An updated version of metacore is available that allows users to "silence" these messages. See GitHub Issue: https://github.com/atorus-research/metacore/pull/138
