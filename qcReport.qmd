...
for (y in input_dataset_names) {
  ...
  if ("adadas" %in% input_dataset_names) {
    diffs <- tryCatch({
      diffdf(get(new_dataset), get(comp_dataset),
             keys = c("USUBJID", "PARAMCD", "AVISIT", "ADT"), tolerance = .01)
    }, error = function(e) {
      message("Error in diffdf with keys: ", e$message)
      message("Column names in new_dataset:")
      print(names(get(new_dataset)))
      message("Column names in comp_dataset:")
      print(names(get(comp_dataset)))
